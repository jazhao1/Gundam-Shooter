...
# player variables
player = gundam.Gundam(100, display_height/2-40)
moving = True
godmode = False
...
# enemy gundam variables
enemy_suits = enemy_suits.Enemysuits(-100, display_height/2-40)
enemy_suits_alive = False
    ...
    while menu:
        ...
        if cloud_x <= 800 - 1100:
            ...
            if not player.wreck_start:
                cloud_x -= 5
        if godmode:
            title = message_to_screen("GUNDAM (GODMODE)", font, 80, yellow)
        else:
            title = message_to_screen("GUNDAM", font, 100, black)
        controls_1 = message_to_screen("use WASD to move, SPACE to shoot, M to drop bombs", font, 30, black)
        controls_2 = message_to_screen("Score over 5,000 to WIN", font, 30, black)
        controls_3 = message_to_screen("Code/tutorial by andreasroald", font, 30, black)
        ...
        game_display.blit(controls_3, (display_width/2 - (controls_3_rect[2]/2), 160))
        ...
        pygame.display.set_caption("GUNDAM running at " + str(int(clock.get_fps())) + " frames per second.")
        clock.tick(FPS)
...
def game_loop():
    ...
    while not game_exit:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_SPACE or event.key == pygame.K_RETURN:
                        ...
                        if game_over_selected == "quit":
                            ...
                            quit()
            if score >= win_score:
                game_over_text = message_to_screen("YOU WIN", font, 100, black)
            else:
        ...
        # drawing enemy gundam
        game_display.blit(enemy_suits.current, (enemy_suits.x, enemy_suits.y))
        ...
        if not player.wreck_start and not player.wrecked:
            for draw_bullet in bullets:
                pygame.draw.rect(game_display, pink, (draw_bullet[0]+90, draw_bullet[1]+40, 10, 10))
        ...
        if not player.wreck_start and not player.wrecked:
            ...
            for move_bomb in range(len(bombs)):
                bombs[move_bomb][0] += 10
                bombs[move_bomb][1] += 10
        ...
        if not player.wreck_start and not player.wrecked and not game_over:
            for draw_bullet in enemy_suits.bullets:
            ...
            for move_bullet in range(len(enemy_suits.bullets)):
                enemy_suits.bullets[move_bullet][0] -= 15
            for del_bullet in enemy_suits.bullets:
                if del_bullet[0] <= -40:
                    enemy_suits.bullets.remove(del_bullet)
        ...
        # spawn enemy gundam randomly
        enemy_spawn_num = random.randint(0, 100)
        if not enemy_suits_alive and score > 250 and enemy_spawn_num == 50:
            enemy_suits_alive = True
            enemy_suits.x = 800
        ...
        for hit_enemy_suits in bullets:
            if enemy_suits.x < hit_enemy_suits[0]+90 < enemy_suits.x+100 \
               or enemy_suits.x < hit_enemy_suits[0]+100 < enemy_suits.x+100:
                if enemy_suits.y < hit_enemy_suits[1]+40 < enemy_suits.y+80 \
                   or enemy_suits.y < hit_enemy_suits[1]+50 < enemy_suits.y+80:
                    if not enemy_suits.x > 600:
                        pygame.mixer.Sound.play(explosion2)
                        score += 150
                        bullets.remove(hit_enemy_suits)
                        enemy_suits.x = -100
                        enemy_suits_alive = False
        ...
        for hit_player in enemy_suits.bullets:
            if player.x < hit_player[0] < player.x+100 or player.x < hit_player[0]+40 < player.x+100:
                if player.y < hit_player[1]+40 < player.y+80 or player.y < hit_player[1]+50 < player.y+80:
                    ...
                    player.damaged = True
                    player.health -= 1
                    enemy_suits.bullets.remove(hit_player)
        ...
        if score < highscore_int:
            ...
            hi_score_message = message_to_screen("Score {0} to win".format(win_score), font, 50, black)
        else:
            highscore_file = open('highscore.dat', "w")
            highscore_file.write(str(score))
            highscore_file.close()
